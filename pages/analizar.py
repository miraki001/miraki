...
def ingresar(vtitu,vfuente,vproyecto,vfuente_nuri,vdet,vlink,veje,vimg,vpeso,vtipo):
      with conn.session as session:
        ...
        actualiza = actualiza + "VALUES (nextval('novedades_seq'), :fuente, ltrim(rtrim(:titulo)),ltrim(rtrim(:detalle)),:tipo,current_date,:selec,:origen,:link,:tema,:proyecto_nuri,:web,:alerta,:imagen,:fuente_nuri,:eje_nuri,:nro,:leido,:puntaje);"
...
for index in range(len(df)) :
   ...
   p = int((index *100) / len(df))
   progress_text = 'Fuente :  '  + fuente + ' ' + str(index+1) + ' de ' + str(len(df))
