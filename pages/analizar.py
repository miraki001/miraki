...
def ingresar(vtitu,vfuente,vproyecto,vfuente_nuri,vdet,vlink,veje,vimg,vpeso):
      with conn.session as session:
        ...
        session.execute(text(actualiza), {"fuente": vfuente,"titulo": vtitu,"detalle": vdet,"tipo": 'N',"selec": 'N',"origen": 'A',"link": vlink,"tema": '',"proyecto_nuri": vproyecto,"web": 'N',"alerta": 'N',"imagen": vimg,"fuente_nuri": vfuente_nuri,"eje_nuri": veje,"nro": 5,"leido": 'N',"puntaje": vpeso})
        session.commit()
activa = "S"
...
for index in range(len(df)) :
   ...
   for index in range(len(dres)) :
      ...
      if buscar_pers == 'S' and peso > 3:
        ingresar(tit,fuente,int(vpro),int(fnuri),det,link,eje,img,peso)
        vcnt1 = vcnt1 + 1
      if buscar_pers == 'N':
        ingresar(tit,fuente,int(vpro),int(fnuri),det,link,eje,img,peso)
        vcnt1 = vcnt1 + 1
